...
# $Header: /var/cvsroot/gentoo-x86/sys-apps/portage/portage-2.2_rc31.ebuild,v 1.1 2009/04/19 23:28:13 zmedico Exp $
...
src_unpack() {
	...
	epatch /usr/local/portage/my_added/sys-apps/portage/files/notitles.patch
}
...
pkg_preinst() {
	...
	[[ -n $PORTDIR_OVERLAY ]] && has_version "<${CATEGORY}/${PN}-2.2_rc29"
...
pkg_postinst() {
	...
	if [ $REPO_LAYOUT_CONF_WARN = 0 ] ; then
		...
		echo "If you want overlay eclasses to override eclasses from" \
			...
			"configuration files." \
			| fmt -w 75 | while read ; do ewarn "$REPLY" ; done
		ewarn
	fi
