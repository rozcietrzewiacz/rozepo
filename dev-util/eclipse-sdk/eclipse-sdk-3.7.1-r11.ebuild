...
src_install() {
	...
	for xLib in atk-gtk gtk pi-gtk ; do
		...
		local xTgt=$( echo ${xLibDir}/libswt-${xLib}-*.so | awk '{ print $NF; }' )
		[[ "$( echo ${xLibDir}/libswt-${xLib}-*.so )" != "$xTgt" ]] && \
			ewarn "Found more than one version of libswt installed!" && \
			ewarn "Linking ${xSrc} against ${xTgt}"
		xTgt=$( basename ${xTgt} )
		dosym "${xTgt}" "${xSrc}" || die
	done
	eend
